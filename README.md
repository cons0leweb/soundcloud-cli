...
- бесконечное радио по любому треку с рекомендациями SoundCloud и автопополнением очереди (`a`);
...
- бесконечная SoundCloud-style waveform: уникальная для каждого трека, движется при воспроизведении и замирает на паузе;
...
```bash
curl -LO https://github.com/cons0leweb/soundcloud-cli/releases/download/v0.3.0/soundcloud_0.3.0_Linux_x86_64.tar.gz
tar -xzf soundcloud_0.3.0_Linux_x86_64.tar.gz
